...
interface Props {
    record: ListadoProps
    handleDeleteListado: (listado: ListadoProps) => void
    handleChangeListado: (listado: ListadoProps, e: React.FocusEvent<HTMLInputElement, Element>) => void
    handleChangeColor: (listado: ListadoProps, color: string) => void
    isClosed?: boolean
}

export const TaskListadoHeader = ({ record, handleDeleteListado, handleChangeListado, handleChangeColor, isClosed }: Props) => (
        ...
        >
            ...
            <Form.Item
                ...
                <Input
                    ...
                    onBlur={ e => handleChangeListado(record, e) } 
                    name='titulo'
                    onPressEnter={ (e) => {
                            e.preventDefault()
                            e.stopPropagation()
                ...
                <Tooltip title="Eliminar Sección" >
                    <Popconfirm
                        title="¿Estás seguro de eliminar esta Sección?"
                            onConfirm={() => handleDeleteListado(record)}
                            onCancel={() => {}}
                            okText="Si"
                            cancelText="No"
                            placement="left"
